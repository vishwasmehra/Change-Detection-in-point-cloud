...
#include <C:\opencv\build\install\include\opencv2\core/core.hpp>
...
//	ui->setupUi(this);
    ...
    QMenu* submenuA = menu->addMenu( "Registration Tools" );
    QAction *reference = new QAction("Reference Image",this);
     QAction *alignPair = new QAction("Align Point Pair Registration",this);
      QAction *fineRegister = new QAction("Fine Registration",this);
    submenuA->addAction(reference);
    submenuA->addAction(alignPair);
    submenuA->addAction(fineRegister);
    QObject::connect(reference, SIGNAL(triggered()),
                                    this, SLOT(Open_clicked()));
    QObject::connect(alignPair, SIGNAL(triggered()),
                                    this, SLOT(Registration_clicked()));
    QObject::connect(fineRegister, SIGNAL(triggered()),
                                    this, SLOT(doActionRegister()));
    Registration->setMenu(menu);
    //Console
    ...
    setWindowTitle(QString("DHAROHAR"));
    ...
    ccConsole::Print("Dharohar started!");


    Open->setStyleSheet("background-color:lightgray;" "display:inline-block;" "border: 1px solid black; " );
   // Reference->setStyleSheet("background-color:lightgray;" "display:inline-block;" "border: 1px solid black; " );
    Registration->setStyleSheet(" background-color:lightgray;" "border: 1px solid black; " );
    Translate->setStyleSheet(" background-color:lightgray;" "border: 1px solid black; " );
    Unroll->setStyleSheet(" background-color:lightgray;" "border: 1px solid black; " );
    Rasterize->setStyleSheet(" background-color:lightgray;" "border: 1px solid black; " );
    Crack->setStyleSheet("background-color:lightgray;" "border: 1px solid black; ");
  //  Store->setStyleSheet(" background-color:lightgray;" "border: 1px solid black; " "border-radius: 15px;");
...
void MainWindow::connectActions()
{
    ...
    //connect(Reference, SIGNAL(clicked()), this, SLOT(Open_clicked()));
    connect(Registration, SIGNAL(clicked()), this, SLOT(Registration_clicked()));
    connect(Translate, SIGNAL(clicked()), this, SLOT(Translate_clicked()));
    connect(Unroll, SIGNAL(clicked()), this, SLOT(Unroll_clicked()));
    connect(Rasterize, SIGNAL(clicked()), this, SLOT(Rasterize_clicked()));
    connect(Crack, SIGNAL(clicked()), this, SLOT(doActionCloudCloudDist()));

    /*** MAIN MENU ***/

    connect(actionCropping_2,                            SIGNAL(triggered()),    this,        SLOT(activateClippingBoxMode()));
    connect(actionSegmentation,                            SIGNAL(triggered()),    this,        SLOT(activateSegmentationMode()));
    connect(actionMerge_2,                            SIGNAL(triggered()),    this,        SLOT(doActionMerge()));
    ...
    connect(actionGlobalShiftSettings,			SIGNAL(triggered()),	this,		SLOT(doActionGlobalShiftSeetings()));
     connect(actionCropping,                    SIGNAL(triggered()),	this,		SLOT(activateClippingBoxMode()));
    ...
    connect(actionRegister_2,						SIGNAL(triggered()),	this,		SLOT(doActionRegister()));
   ...
   // connect(actionImage_Analysis,				SIGNAL(triggered()),	this,		SLOT(actionImage_Analysis_triggered()));

    //Change Detection

  //  connect(actionLoad_Image1,				SIGNAL(triggered()),	this,		SLOT(actionLoad_Image1_triggered()));
//    connect(actionLoad_Image2,				SIGNAL(triggered()),	this,		SLOT(actionLoad_Image2_triggered()));
    //connect(actionChange_Detection,				SIGNAL(triggered()),	this,		SLOT(actionChange_Detection_triggered()));
    //connect(actionTexture_Analysis_3,				SIGNAL(triggered()),	this,		SLOT(actionTexture_Analysis_2_triggered()));
...
void MainWindow::doActionRegister()
{
    ...
    refreshAll();
    updateUI();
     Registration->setStyleSheet("background-color:rgb(70,0,0);" " color:white;" "display:inline-block;" "border: 1px solid black;");
}
...
void MainWindow::doActionResetGUIElementsPos()
{
    ...
    QMessageBox::information(this,"Restart","To finish the process, you'll have to close and restart Dharohar");
...
{
   QMessageBox::information(this, "Documentation", "Please visit help document in the main window");

    //QProcess *myProcess = new QProcess(this);

    //myProcess->startDetached("document/qt_user_manual.pdf");
    //QDesktopServices::openUrl(QUrl("document/qt_user_manual.pdf"));
...
void MainWindow::deactivateRegisterPointPairTool(bool )
{
    if (m_pprDlg)
        m_pprDlg->clear();
...
void MainWindow::deactivateSectionExtractionMode(bool )
{
    if (m_seTool)
...
void MainWindow::deactivatePointListPickingMode(bool )
{
    if (m_plpDlg)
    {
        //m_plpDlg->linkWith(0);
...
void MainWindow::deactivateClippingBoxMode(bool )
{
...
void MainWindow::deactivateTranslateRotateMode(bool state)
{
    ...
    updateUI();
     Translate->setStyleSheet("background-color:rgb(70,0,0);""color:white;" "display:inline-block;" "border: 1px solid black; ");
}
...
void MainWindow::enableUIItems(dbTreeSelectionInfo& selInfo)
{
    ...
    actionRegister_2->setEnabled(exactlyTwoEntities);
...
//#endif
    ...
    //load files
    addToDB(selectedFiles,currentOpenDlgFilter);
    Open->setStyleSheet("background-color:rgb(70,0,0);" " color:white;" "display:inline-block;" "border: 1px solid black; " );
    value=value+1;
    //if(value==2)
        //Reference->setStyleSheet("background-color:green;" " color:white;" "display:inline-block;" "border: 1px solid black; ");

}

void MainWindow::Unroll_clicked()
{
    ...
    if (output)
    {
     ...
     Unroll->setStyleSheet("background-color:rgb(70,0,0);""color:white;" "display:inline-block;" "border: 1px solid black;");
}
...
void MainWindow::Rasterize_clicked()
{
    size_t selNum = m_selectedEntities.size();
    if (selNum != 1)
    {
    ...
    ccRasterizeTool rasterizeTool(cloud,this);
    rasterizeTool.exec();
    Rasterize->setStyleSheet("background-color:rgb(70,0,0);" " color:white;"  "display:inline-block;" "border: 1px solid black; ");
}

void MainWindow::Registration_clicked()
{
